...
namespace _Scripts.Bot_Scene_Scripts {
    public class WinControllerBP : MonoBehaviour {
            ...
            if (!IsWin) {
                for (int i = 0; i < Length; i++) {
                    ...
                    if (_firstPlayerPos.x >= _vertexPos1.x - 0.01 && _firstPlayerPos.x <= _vertexPos1.x + 0.01 &&
                        ...
                        IsWin = true;
                        winText.text = GetInfo(firstPlayer.GetComponent<BotOrPlayer>().PlayerType);
                        break;
                    }
                    if (_secondPlayerPos.x >= _vertexPos2.x - 0.01 && _secondPlayerPos.x <= _vertexPos2.x + 0.01 &&
                        ...
                        IsWin = true;
                        winText.text = GetInfo(secondPlayer.GetComponent<BotOrPlayer>().PlayerType);
                        break;
                    }
                }
            } else {
                if (winText.transform.localPosition.x >= 60)
                    winText.transform.Translate(Vector3.down * Time.deltaTime * speed);
                if (buttons.transform.localPosition.x <= 0)
                    buttons.transform.Translate(Vector3.right * Time.deltaTime * speed);
            }
        }

        private string GetInfo(BotOrPlayer.Type player) {
